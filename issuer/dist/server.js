...
import { generateEd25519KeyPair, makeDisclosure, digestDisclosure } from '@did-ai/shared';
import { appendFile, readFile } from 'fs/promises';
import { randomUUID } from 'crypto';
// Simple in-memory status list using a BitSet-like array of booleans.
const statusLists = {};
function getOrCreateStatusList(id, size = 10000) {
    if (!statusLists[id])
        statusLists[id] = { purpose: 'revocation', bits: new Uint8Array(size) };
...
const issuerPrivJwk = { kty: 'OKP', crv: 'Ed25519', x: Buffer.from(issuerKeypair.publicKey).toString('base64url'), d: Buffer.from(issuerKeypair.privateKey).toString('base64url'), kid: issuerKid };
const issuerPubJwk = { kty: 'OKP', crv: 'Ed25519', x: Buffer.from(issuerKeypair.publicKey).toString('base64url'), kid: issuerKid };
export function buildServer() {
    ...
    app.post('/oid4vci/issue', async (req, reply) => {
        ...
        if (index === -1)
            return reply.code(503).send({ error: 'no status slots' });
