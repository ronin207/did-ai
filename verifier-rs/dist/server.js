...
const nonces = new Map(); // nonce -> info
const usedJti = new Map(); // jti -> expTs
const statusCache = new Map();
// RS HMAC secret for access tokens (demo)
import { randomBytes } from 'crypto';
const rsSecret = randomBytes(32);
export function buildServer(issuerBase = ISSUER_BASE, issuerClient) {
    ...
    const sweepHandle = setInterval(sweep, 60_000);
    ...
    app.get('/protected/resource', async (req, reply) => {
        ...
        if (!cnfJkt)
            return reply.code(401).send({ error: 'missing cnf' });
        // Validate DPoP proof
        if (!dpop || typeof dpop !== 'string')
            return reply.code(401).send({ error: 'missing DPoP' });
        try {
            const segments = dpop.split('.');
            const header = JSON.parse(Buffer.from(segments[0], 'base64url').toString('utf8'));
            if (!header || header.typ !== 'dpop+jwt' || !header.jwk)
                return reply.code(401).send({ error: 'bad DPoP header' });
            const jwk = header.jwk;
            // Thumbprint must match cnf.jkt
            const jkt = await (await import('jose')).calculateJwkThumbprint(jwk, 'sha256');
            if (jkt !== cnfJkt)
                return reply.code(401).send({ error: 'cnf mismatch' });
            ...
            if (htm !== 'GET' || htu !== expectedHtu)
                return reply.code(401).send({ error: 'htu/htm' });
            const now = Math.floor(Date.now() / 1000);
            if (typeof iat !== 'number' || Math.abs(now - iat) > 120)
                return reply.code(401).send({ error: 'iat' });
            if (!jti || typeof jti !== 'string')
                return reply.code(401).send({ error: 'jti' });
            // Replay cache
            const seen = usedJti.get(jti);
            if (seen && seen > now)
                return reply.code(401).send({ error: 'replay' });
    ...
    app.post('/present', async (req, reply) => {
        ...
        if (!vp_token || !state)
            return reply.code(400).send({ error: 'vp_token and state required' });
        ...
        try {
            const { definition_id, descriptor_map } = presentation_submission;
        ...
        const listPath = status.statusListCredential;
        let sl;
        ...
        if (issuerClient) {
            ...
            const res = await fetch(`${issuerBase}${listPath}`, { headers });
